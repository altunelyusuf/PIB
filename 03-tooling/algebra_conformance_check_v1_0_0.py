#!/usr/bin/env python3
"""
PIB Algebra Conformance Check v1.0.0
====================================
Checks PIB's taxonomy and capacities against the variant algebra's OWN enumerator as the oracle, rather
than against expectations PIB wrote for itself.
...
SPACES = [os.path.join(ROOT, "12-operator-fixtures", "profile_variation_spaces_v1_0_0.ttl"),
...
def main():
    ...
    # Outward: PIB's real spaces written as DSL and enumerated by the algebra.
    print("  PIB's own spaces, written as algebra DSL and counted by the algebra:")
    _spec2 = importlib.util.spec_from_file_location("vc", os.path.join(ROOT, "03-tooling", "variation_capacity_v1_2_0.py"))
    vc = importlib.util.module_from_spec(_spec2); _spec2.loader.exec_module(vc)
    for path in SPACES:
        caps, _, _ = vc.compute([path], verbose=False)
        g = rdflib.Graph(); g.parse(path, format="turtle")
        for sp in sorted((s for s in g.subjects(rdflib.RDF.type, PIBE.VariationSpace)
                          if (s, PIBE.groupOf, None) not in g), key=str):
            name, pib = str(sp).split("#")[-1], caps.get(str(sp))
            if pib is None:
                print(f"    {name:26s} no capacity (empty space) — skipped"); continue
            r = subprocess.run([sys.executable, CONVERTER, "to-dsl", path, name], capture_output=True, text=True)
            try:
                oracle = len(ve.enumerate_variants(parse(r.stdout).components[0]))
            except Exception as e:
                print(f"    {name:26s} algebra could not enumerate: {type(e).__name__}"); disagree += 1; continue
            same = pib == oracle
            agree, disagree = agree + int(same), disagree + int(not same)
            print(f"    {name:26s} PIB {pib:6d} | algebra {oracle:6d} | {'AGREE' if same else 'DISAGREE'}")
